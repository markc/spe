...
final class CategoriesModel extends Plugin {
    ...
    public function __construct(protected Ctx $ctx) {
        ...
        $this->dbh = new Db(['type' => 'sqlite', 'path' => dirname(__DIR__) . '/Posts/blog.db', 'name' => 'blog']);
    }
    ...
    #[\Override] public function update(): array {
        ...
        if (Util::is_post()) {
            ...
            if (!$name) {
                Util::log('Category name is required');
                return array_merge($category, ['name' => $name, 'slug' => $slug, 'description' => $this->in['description']]);
            }

            // Check uniqueness (excluding current)
            $existing = $this->dbh->read('categories', 'id', 'slug = :slug AND id != :id', ['slug' => $slug, 'id' => $id], QueryType::One);
            if ($existing) {
                Util::log('A category with this slug already exists');
                return array_merge($category, ['name' => $name, 'slug' => $slug, 'description' => $this->in['description']]);
            }

            $this->dbh->update('categories', [
                'name' => $name,
                'slug' => $slug,
                'description' => $this->in['description'],
                'updated' => date('Y-m-d H:i:s')
    ...
    #[\Override] public function list(): array {
        ...
        foreach ($categories as &$cat) {
            $count = $this->dbh->read('post_categories', 'COUNT(*)', 'category_id = :id', ['id' => $cat['id']], QueryType::Column);
