...
use SPE\Users\Core\{Ctx, Db, Plugin, QueryType};
...
final class UsersModel extends Plugin {
    ...
    public function __construct(protected Ctx $ctx) {
        ...
        if (is_null($this->dbh)) {
            $this->dbh = new Db(['type' => 'sqlite', 'path' => __DIR__ . '/users.db', 'name' => 'users']);
        }
    }
    ...
    #[\Override] public function create(): array {
        if ($_POST) {
            $data = [
                ...
                'altemail' => $this->in['altemail'], 'webpw' => $this->in['webpw'] ? password_hash($this->in['webpw'], PASSWORD_DEFAULT) : '',
                'otp' => $this->in['otp'], 'otpttl' => (int)$this->in['otpttl'], 'cookie' => $this->in['cookie'], 'anote' => $this->in['anote'],
    ...
    #[\Override] public function list(): array {
        ...
        return [
            'items' => $this->dbh->read('users', '*', $where . ' ORDER BY updated DESC, created DESC LIMIT :limit OFFSET :offset', $params, QueryType::All),
