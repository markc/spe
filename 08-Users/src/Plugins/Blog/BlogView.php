...
final class BlogView
{
    ...
    public function read(): string
    {
        $a = $this->a;
        if (!$a)
            return '<div class="card mt-4"><p class=text-muted>Post not found.</p><a href="/blog" class=btn><i data-lucide="arrow-left"></i> Back</a></div>';
        $ico = $this->icon($a['icon'] ?? '');
        $admin = Util::is_adm()
            ? "<a href='?o=Blog&m=update&i={$a['id']}' class=btn><i data-lucide=\"edit\"></i> Edit</a>
            <a href='?o=Blog&m=delete&i={$a['id']}' class='btn btn-danger' onclick='return confirm(\"Delete?\")'><i data-lucide=\"trash-2\"></i></a>" : '';
        return <<<HTML
<div class='card mt-4'>
    <h2>{$ico}{$a['title']}</h2>
    ...
    <div class='flex mt-3 gap-sm justify-end'>
        <a href='/blog' class=btn><i data-lucide="arrow-left"></i> Back</a>
        {$admin}
    </div>
</div>
HTML;
    ...
    public function page(): string
    {
        ...
        $ico = $this->icon($a['icon'] ?? '');
        $admin = Util::is_adm()
            ? "<div class='mt-2 text-right'><a href='?o=Blog&m=update&i={$a['id']}' class=btn><i data-lucide=\"edit\"></i> Edit</a></div>"
            : '';
        return <<<HTML
<div class='card mt-4'>
    <h2>{$ico}{$a['title']}</h2>
...
<article class='card mb-2'>
    <h2 class=m-0><a href='/{$slug}' class=no-underline>{$ti}</a></h2>
    ...
    <p class=m-0>{$ex}</p>
    <div class=text-right><a href='/{$slug}' class=btn>Read More <i data-lucide="arrow-right"></i></a></div>
</article>
HTML;
        }
        $h .= $this->pg($a['pagination'], '');
        $admin = Util::is_adm()
            ? "<div class='text-right mt-2'><a href='?o=Blog&edit' class=btn><i data-lucide=\"settings\"></i> Manage Posts</a></div>"
            : '';
        return $h . "$admin</div>";
    }
...
<div class='card mt-4'>
    <div class='flex justify-between mb-2'>
        ...
        <div class='flex gap-sm'>
            <a href='/blog' class=btn><i data-lucide="newspaper"></i> View Blog</a>
...
HTML;
        foreach ($a['items'] as $i) {
            ...
            $h .= <<<HTML
<tr class=tr>
    <td class=td><a href='/{$slug}'>{$ti}</a></td>
    <td class=td><small><i data-lucide="{$ico}"></i> {$tp}</small></td>
    <td class=td><small>{$i['updated']}</small></td>
    <td class=td-right>
        <a href='?o=Blog&m=update&i={$i['id']}'><i data-lucide="edit"></i></a>
        <a href='?o=Blog&m=delete&i={$i['id']}' onclick='return confirm("Delete?")'><i data-lucide="trash-2"></i></a>
    </td>
</tr>
HTML;
        }
    ...
    private function pg(array $p, string $x): string
    {
        ...
        if ($p['page'] > 1)
            $h .= "<a href='{$base}{$sep}page=" . ($p['page'] - 1) . "$sq' class=btn><i data-lucide=\"chevron-left\"></i> Prev</a>";
        $h .= "<span class=td>Page {$p['page']} of {$p['pages']}</span>";
        if ($p['page'] < $p['pages'])
            $h .= "<a href='{$base}{$sep}page=" . ($p['page'] + 1) . "$sq' class=btn>Next <i data-lucide=\"chevron-right\"></i></a>";
        return $h . '</div>';
    }
        ...
        <div class=flex>
            ...
            <div class=form-group>
                ...
                <select id=type name=type>
                    <option value=post {$ps}>Post</option>
                    <option value=page {$pgs}>Page</option>
                </select>
            </div>
        </div>
        <div class=form-group>
