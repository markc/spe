...
final class Md
{
    ...
    public static function parse(string $s): string
    {
        ...
        $s = preg_replace_callback('/^(\|.+\|)\r?\n(\|[-:\| ]+\|)\r?\n((?:\|.+\|\r?\n?)+)/m', static function($m) use ($L, $R) {
            ...
            $th = implode('', array_map(static fn($h, $i) => "{$L}th style=\"text-align:{$align[$i]}\"{$R}$h{$L}/th{$R}", $hdr, array_keys($hdr)));
            $rows = array_filter(explode("\n", trim($m[3])));
            $tb = implode('', array_map(static function($r) use ($align, $L, $R) {
                $cells = array_map('trim', array_filter(explode('|', $r)));
                return "{$L}tr{$R}" . implode('', array_map(static fn($c, $i) =>
        ...
        // Block elements
        $s = preg_replace_callback('/^(#{1,6})\s+(.+)$/m', static fn($m) => "{$L}h" . strlen($m[1]) . "{$R}" . trim($m[2]) . "{$L}/h" . strlen($m[1]) . "{$R}", $s);
        ...
        // Paragraphs
        return implode("\n", array_map(static fn($p) => ($p = trim($p)) === '' ? '' :
