...
class Theme
{
    ...
    protected function dropdown(string $label, array $items, string $param = 'o'): string
    {
        if (empty($items))
            return '';
        $links = $items
            |> (static fn($a) => array_map(static fn($n) => sprintf(
                '<a href="%s">%s</a>',
                str_starts_with($n[1], '?') ? $n[1] : "?{$param}={$n[1]}",
                $n[0],
            ), $a))
    ...
    {
        return $_GET |> (static fn($p) => [...$p, 't' => $theme]) |> http_build_query(...) |> (static fn($q) => "?$q");
    }
    ...
    protected function authNav(): string
    {
        if (!empty($_SESSION['authenticated'])) {
            ...
            $img = $thumb
                ? "<img src=\"$thumb\" alt=\"$name\" style=\"width:28px;height:28px;border-radius:50%;vertical-align:middle;margin-right:0.5rem\">"
                : '';
            return <<<HTML
