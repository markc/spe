...
final class Util {
    ...
    public static function toast(): string {
        ...
        $class = $isSuccess ? 'toast-success' : 'toast-danger';
        $bg = $isSuccess ? '#d4edda' : '#f8d7da';
        $color = $isSuccess ? '#155724' : '#721c24';

        return <<<HTML
        <div class="toast {$class}" style="margin-bottom:1rem;padding:1rem;border-radius:4px;background:{$bg};color:{$color}">
            {$msg}
        </div>
        HTML;
    }
