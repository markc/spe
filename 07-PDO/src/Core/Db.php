...
class Db extends PDO {
    ...
    public function create(string $table, array $data): int {
        $fields = array_keys($data) |> (fn($k) => implode(', ', $k));
        $values = array_keys($data) |> (fn($k) => array_map(fn($x) => ":$x", $k)) |> (fn($v) => implode(', ', $v));
    ...
    public function update(string $table, array $data, string $where, array $params = []): bool {
        $set = array_keys($data) |> (fn($k) => array_map(fn($x) => "$x = :$x", $k)) |> (fn($s) => implode(', ', $s));
